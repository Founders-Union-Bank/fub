...
public interface PaymentDetailAPI {
    ...
    @ApiResponse(responseCode = "200", description = "Fetch user payment details Success", content = @Content(schema = @Schema(implementation = PaymentDetailModel.class)))
    @GetMapping(value = "/paymentDetails", produces = {"application/json"})
    ResponseEntity<List<PaymentDetail>> fetchPaymentsDetails(
            @Parameter(in = ParameterIn.QUERY, description = "User of the payment") @NotNull @RequestParam(value = "userId") String userId,
                @Parameter(in = ParameterIn.QUERY, description = "Crew Id") @RequestParam("crewId") @NotNull Long crewId
    );
    ...
    ResponseEntity<PaymentDetail> updatePaymentDetails(
            ...
            @Parameter(in = ParameterIn.PATH, description = "Id of the payment") @NotNull @PathVariable(value = "paymentId") String paymentId,
            @Parameter(in = ParameterIn.QUERY, description = "Crew Id") @RequestParam("crewId") @NotNull Long crewId,
            @Parameter(in = ParameterIn.DEFAULT, description = "Body of the payment") @Valid @NotNull @RequestBody PaymentDetail paymentDetail
    );

    @Operation(summary = "Generate Payment details for members", description = "Generate payment details for all the members for part of the group")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Payment details updated successfully"),
            @ApiResponse(responseCode = "404", description = "Group not Found")
    })
    @PostMapping(value = "/paymentDetails/generate-payment")
    ResponseEntity<String> generatePaymentDetailsForMembers(
            @Parameter(in = ParameterIn.QUERY, description = "Crew Id") @RequestParam("crewId") @NotNull Long crewId
    );
